            ...
            <div class="columns is-centered">
                <div class="column is-6">
                    <div class="box">
                        ...
                        <div class="has-text-weight-bold is-size-4">CURRICULUM</div>
                        ...
                        <b-table
                            ...
                            <b-table-column field="curriculum_id" label="ID" sortable v-slot="props">
                                {{ props.row.curriculum_id }}
                            </b-table-column>
                            ...
                            <b-table-column field="curriculum_description" label="Description" sortable v-slot="props">
                                {{ props.row.curriculum_description }}
                            </b-table-column>
            ...
            <form @submit.prevent="submit">
                <div class="modal-card">
                    <header class="modal-card-head">
                        <p class="modal-card-title has-text-weight-bold is-size-6">Add/Update Curriculum</p>
                        <button
                            type="button"
                            class="delete"
                            ...
                            <div class="columns">
                                <div class="column">
                                    <b-field label="Description" label-position="on-border"
                                        :type="this.errors.curriculum_description ? 'is-danger':''"
                                        :message="this.errors.curriculum_description ? this.errors.curriculum_description[0] : ''">
                                        <b-input v-model="fields.curriculum_description"
    ...
    methods: {
        ...
        */
        loadAsyncData() {
            const params = [
                ...
                `curriculum=${this.search.curriculum}`,
                `perpage=${this.perPage}`,
                `page=${this.page}`
            ].join('&')

            this.loading = true
            axios.get(`/get-curriculums?${params}`)
                .then(({ data }) => {
                    this.data = [];
        ...
        */
        ...
        submit: function(){
            if(this.global_id > 0){
                //update
                axios.put('/manage-curriculums/'+this.global_id, this.fields).then(res=>{
                ...
                //INSERT HERE
                axios.post('/manage-curriculums', this.fields).then(res=>{
        ...
        deleteSubmit(delete_id) {
            axios.delete('/manage-curriculums/' + delete_id).then(res => {
                this.loadAsyncData();
                this.clearFields()
            }).catch(err => {
                if (err.response.status === 422) {
        ...
        getData: function(data_id){
            ...
            axios.get('/manage-curriculums/'+data_id).then(res=>{
